...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Añadir búsqueda actual al historial
        <?php if (isset($_POST['codigo']) && !empty($_POST['codigo'])): ?>
            const nuevaBusqueda = {
                codigo: "<?= htmlspecialchars($_POST['codigo']) ?>",
                encontrado: <?= $producto ? 'true' : 'false' ?>,
                timestamp: new Date().toISOString(),
                nombre: <?= $producto ? '"' . htmlspecialchars($producto['nombre']) . '"' : 'null' ?>
            };
        ...
        function mostrarHistorial() {
            ...
            historialBusquedas.forEach(item => {
                ...
                <td>
                    <button type="button" class="btn btn-primary btn-sm btn-buscar-codigo" data-codigo="${item.codigo}">
                        <i class="fas fa-search"></i> Buscar
                    </button>
                </td>
            </tr>`;
