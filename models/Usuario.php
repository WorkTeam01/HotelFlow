...
class Usuario
{
    ...
    public function crear($datos)
    {
        try {
            ...
            $query = "INSERT INTO {$this->tabla} (nombre, apellidop, apellidom, tipodocumento, numdocumento, 
                      direccion, telefono, correo, cargo, clave, imagen, estado) 
                      VALUES (:nombre, :apellidop, :apellidom, :tipodocumento, :numdocumento, 
                      :direccion, :telefono, :correo, :cargo, :clave, :imagen, :estado)";
    ...
    {
        try {
            $query = "SELECT * FROM {$this->tabla} WHERE correo = :correo AND estado = 1";
            $stmt = $this->conexion->prepare($query);
            $stmt->bindParam(':correo', $correo, PDO::PARAM_STR);
            $stmt->execute();
