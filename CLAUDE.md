...
- **Escapar salida HTML dinámica:** usar `htmlspecialchars()` al imprimir valores que vienen de BD/usuario en vistas (ej. `$venta['metodopago']`). En PHP 8.1+ **nunca pasar `null` crudo**: siempre `htmlspecialchars($valor ?? '')` (o un fallback de negocio `?? 'N/A'`) cuando la columna es nullable (`persona.telefono`, `precio_equipaje.descripcion`, textos opcionales, etc.) — de lo contrario emite deprecation `Passing null to parameter #1 ($string)` en cada render y, si `display_errors` está activo, el mensaje se cuela dentro del atributo/etiqueta y rompe la estructura HTML. Se encontró y corrigió en `views/almacenamiento-equipaje` (select de clientes con `telefono` NULL en BD) tras un reporte de "teléfono no carga al seleccionar cliente". Si el dato nullable debe mostrarse con contenido real (no vacío), combinar `?? ''` en el escape con un fallback en el markup o en el JS de lectura (`attr(...) || 'N/A'`); si la tabla de prueba tiene el campo NULL, poblarlo para poder verificar el flujo.
...
- **Folio de recepción (`pagos`) como libro mayor, `recepcion.montototal`/`montopagado` como cache:** desde el refactor PMS de `recepcion` (ver `REFACTOR_RECEPCION_PMS.md`), la tabla `pagos` (columnas `tipo` `cargo|pago|reverso`, `concepto`, `idusuario`, `id_pago_reversado`) es la fuente de verdad del folio de un huésped — append-only, nunca se hace `UPDATE` sobre un monto ya escrito; anular es insertar un reverso. `recepcion.montototal`/`montopagado` son un cache recalculado dentro de la misma transacción cada vez que se inserta una línea en `pagos` (vía `Pago::registrarLinea()`/`registrarPago()`/`registrarCargo()`, y también inline en `Recepcion::cambiarHabitacion()` al cargar la diferencia de tarifa). Ninguna escritura a esas dos columnas debe hacerse fuera de ese flujo — el método genérico `actualizar()` de `Recepcion.php` todavía acepta `montototal`/`montopagado` en su mapeo de campos dinámico (deuda técnica preexistente, no introducida por el refactor del folio); no pasar esas claves a `actualizar()`, usar siempre el camino de `pagos`.

- **Orden canónico de bloqueo en operaciones concurrentes de `recepcion`:** toda operación que toque el estado de una recepción y el de su(s) habitación(es) en la misma transacción bloquea primero `recepcion` (`FOR UPDATE` vía `Recepcion::getByIdForUpdate()`) y después `habitaciones` — nunca al revés, para no deadlockear entre `crear()`, `actualizar()`, `cambiarEstado()` y `cambiarHabitacion()`. Cuando se bloquean dos habitaciones a la vez (cambio de habitación), se bloquean en orden ascendente por `id_habitacion`, no en el orden origen→destino, por la misma razón.

- **Partials de vista con docblock de dependencias:** los partials en `views/[modulo]/partials/*.php` (patrón usado primero en `sidebar.php`, luego en `folio.php`, `cambio-habitacion.php`, `tile-habitacion.php`, `tab-*.php`, `fila-movimiento.php`, `kpi-bar.php` de `recepcion`) documentan en un docblock al inicio del archivo qué variables debe definir el include-r ANTES del `include`/`require` (globals como `$URL`, y las variables de datos ya resueltas por el controlador). Un partial solo pinta HTML, nunca contiene lógica de negocio ni hace queries ni `switch`/`usort`/agrupaciones — eso vive en el controlador que arma los datos.

- **Estado de recepción: helper único, no `switch` en la vista.** Etiqueta, clase de color (sufijo AdminLTE), clase de badge e icono FontAwesome de cualquier estado (de BD o derivado) salen **solo** de `RecepcionController::estadoRecepcion(string $estado): array` (`['label','clase','badge','icono','orden']`). Los estados derivados (`no_show`, `salida_vencida`) los resuelve `RecepcionController::estadoDerivado(array $recepcion): string` contra la hora actual. Ninguna vista/partial de `recepcion` vuelve a escribir un `switch` sobre `$recepcion['estado']` ni a hardcodear un `badge-*` de estado — se pasó de 3 vocabularios de color incoherentes (lista/show/tile) a uno solo en el refactor frontend de 2026-08-27. Lenguaje de color canónico: verde=disponible, amarillo=ocupada, azul=reservada, gris=por limpiar, rojo=mantenimiento/vencido.

- **Módulo `recepcion` = una vista con tabs, no varias.** `views/recepcion/index.php` es la única vista de panel (tabs Bootstrap `#hoy` / `#mapa` / `#historial`, hash persistido en `sessionStorage` por `index-recepciones.js`); `lista-recepciones.php` se eliminó y su DataTable es `partials/tab-historial.php`. El controlador expone `panel()` (orquesta `hoy()`+`mapa()`+`kpis()`+`historial()`, único que dispara `liberarNoShows()`), llamado **una sola vez** por carga. El room rack denso es `partials/tile-habitacion.php` (un tile parametrizado `col-6 col-sm-4 col-md-3 col-lg-2`, doble dimensión ocupación+housekeeping) — reemplazó las 5 ramas de `card-habitacion.php`.
...
- **Áreas táctiles ≥44px en botones sin `.input-group`: clase `.rec-touch`.** Para botones de acción en `btn-group` (historial) o sueltos (folio) donde no aplica el wrapper `.input-group-text`, usar `.rec-touch` (pseudo-elemento `::before` absoluto con `max(100%,44px)`, mismo patrón que `.rec-tile__action`/`.rec-actionbar`) + `min-width:2.5rem`. No forzar `min-height` en la caja visible. El módulo `habitaciones` tiene su equivalente `.hab-touch` en `public/css/modules/habitaciones/habitaciones.css` (mismo `::before`, `min-width:44px` para cumplir también el ancho AAA 2.5.5 en los botones-ícono del `btn-group` del listado).

- **Estado de habitación: helper único `HabitacionController::estadoHabitacion(string): array`.** Mismo patrón que `RecepcionController::estadoRecepcion()`: etiqueta, clase de color (sufijo AdminLTE), `badge`, icono FontAwesome y `orden` de cualquier estado (`disponible`/`ocupada`/`limpieza`/`mantenimiento`) salen **solo** de ese helper — ninguna vista/JS del módulo vuelve a escribir un `switch` sobre `$habitacion['estado']` ni a hardcodear un `badge-*`. `estadosHabitacion()` devuelve el mapa completo para pintar filtros y acciones. Los estados del **historial** de `show.php` (ocupación: `reservado`/`en_curso`/`finalizado`/`cancelado`; limpieza: `pendiente`/`enprogreso`/`completada`/`verificada`) salen de `HabitacionController::badgeEstadoOcupacion()` / `badgeEstadoLimpieza()` (`['badge','label']`), no de un `switch` en la vista. Vocabulario de color alineado con `recepcion`: verde=disponible, amarillo=ocupada, gris=por limpiar, rojo=mantenimiento. Es un helper aparte de `estadoRecepcion()` a propósito (vocabularios distintos).

- **Estado de equipaje: helper único `AlmacenamientoEquipajeController::estadoEquipaje(string): array`.** Mismo patrón que `estadoRecepcion()`/`estadoHabitacion()`: etiqueta, clase de color, `badge`, icono y `orden` de cualquier estado (`almacenado`/`retirado`/`perdido`/`dañado`/`vencido`) salen **solo** de ese helper. El estado **`vencido` es derivado, no un valor de BD**: `estadoDerivado()` lo resuelve para un `almacenado` con más de `equipaje.dias_alerta` días (`EQUIPAJE_DIAS_ALERTA`, default 7) contra la fecha actual; `decorarEstados()` agrega `estado_ui` (+ `tiempo_almacenado`) a cada fila antes de pintar. `estadosEquipaje()` devuelve solo los estados operables para filtros/dropdowns. Ninguna vista ni JS del módulo vuelve a escribir un `switch` sobre `$equipaje['estado']`. Vocabulario: amarillo=almacenado, verde=retirado, rojo=perdido/vencido, gris=dañado.

- **Folio de equipaje (`pagos.idequipaje`) como libro mayor de solo lectura.** Al crear, `AlmacenamientoEquipaje::crear()` inserta el equipaje y, en la misma transacción, dos líneas de folio vía `Pago::registrarLineaEquipaje()` (cargo `OTROS` + pago con el método elegido), con el monto calculado desde el precio real en BD (`getPrecioEquipaje()`, nunca del POST). La pestaña Folio de `show.php` (`partials/folio-equipaje.php`, datos de `Pago::getByEquipaje()`) es de solo lectura — no debe reaparecer ningún endpoint de alta de cargos/pagos desde la UI de equipaje. `update.php` no edita monto/cantidad/tipo/código (fijos tras el cobro); solo campos descriptivos.

- **Cambio de estado de habitación: flujo único `public/js/modules/habitaciones/cambiar-estado-habitaciones.js`.** Listado (`index.php`) y detalle (`show.php`) comparten el mismo flujo: handler delegado sobre `.cambiar-estado` (con `data-id`/`data-estado`/`data-estado-actual`) → confirmación SweetAlert → `POST` a `controllers/habitaciones/cambiar_estado.php` → `window.location.reload()` en éxito (no se reconstruye la fila a mano). No debe reaparecer el modal `#modalCambiarEstado` ni un `show-habitaciones.js` con su propia copia del flujo.

- **Cache-busting de assets (`views/layouts/header.php` + `footer.php`):** cada entrada de `$module_styles`/`$module_scripts` **y también `public/css/core/styles.css`** se sirve con `?v=<filemtime del archivo>` (fallback `$APP_VERSION`) para que el navegador no sirva CSS/JS viejo tras una edición. No añadir un `?v=` manual en las vistas ni versionar a mano — el layout lo hace por archivo.

- **Tabs inactivos coloreados por clase de card (global, no por módulo):** el texto de las `.nav-tabs .nav-link:not(.active)` hereda la familia de color de la card ancestro (`card-primary`, `card-info`, `card-success`, … — funciona con `card-outline-tabs` y `card-tabs`, y con la clase de color puesta en `.card` o en `.card-header`) vía reglas globales en `public/css/core/styles.css`. AdminLTE solo pinta el `border-top` del tab activo; el texto inactivo se hardcodeaba por módulo (`show-habitaciones.css`, `show-producto.css`, `usuarios.css` con `#detail-tabs`/`#habitacionTabs`) — esos overrides se eliminaron. **No recrear overrides por módulo ni por ID**: si una card nueva necesita otro tono, añadir la regla en `styles.css`. Los colores son variantes oscurecidas de la paleta AdminLTE (misma familia de tono) para cumplir contraste ≥4.5:1 sobre header blanco — los crudos (`#17a2b8` 3.04:1, `#007bff` 3.98:1, `#28a745` 3.13:1, `#ffc107` 1.63:1) no llegan a AA.
