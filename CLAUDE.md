...
- **Partials de vista con docblock de dependencias:** los partials en `views/[modulo]/partials/*.php` (patrón usado primero en `sidebar.php`, luego en `card-habitacion.php`, `paso-*.php`, `folio.php`, `cambio-habitacion.php` de `recepcion`) documentan en un docblock al inicio del archivo qué variables debe definir el include-r ANTES del `include`/`require` (globals como `$URL`, y las variables de datos según el modo/paso). Un partial solo pinta HTML, nunca contiene lógica de negocio ni hace queries — eso vive en el controlador que arma los datos.
